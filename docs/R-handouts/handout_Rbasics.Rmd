...
- **Indentation**: 4 spaces, never tabs (Tabs used in RStudio convert to spaces by default; see RStudio settings for adjusting the number of spaces one tab equals to. This can be also set up for other IDEs. The tidyverse guide suggests two spaces for indentation. We use four throughout this course, including in the Python materials, so that you have one habit rather than two.)
- **Assignment**: use `<-`, not `=`. Note that `=` inside a function call, as in `mean(x = 1:10)`, is not assignment at all — it matches an argument by name, and is correct there.
- **Spacing**: put spaces around binary operators (`+`, `-`, `<-`, `==`, and `=` when naming arguments). No space before a comma, always one after. A space before a left parenthesis, except in a function call — so `if (x > 0)` but `mean(x)`.
